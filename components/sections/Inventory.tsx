...
const InventoryItemCard = memo(({ fragrance }: { fragrance: Fragrance }) => (
  <div className="group relative p-6 bg-white/5 border border-white/10 hover:border-magma/30 rounded-xl transition-colors duration-300">
    <div className="flex justify-between items-start mb-4">
      ...
      <span className="font-sans text-magma font-bold text-base">₹{fragrance.price}</span>
    </div>
    
    <div className="flex flex-wrap gap-2 mb-6">
      {fragrance.notes.map(note => (
        <span key={note} className="px-2 py-1 text-xs font-sans uppercase tracking-wider bg-white/5 rounded text-taupe">
          {note}
        </span>
      ))}
    </div>
...
const Inventory = () => {
  ...
  return (
    <section id="inventory" className="w-full min-h-screen pt-32 px-8 pb-64 bg-obsidian border-t border-white/5">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-end mb-12">
          <ConvergenceWrapper>
            <h2 className="font-serif text-5xl text-bone leading-tight">
              FRAGRANCE <span className="block text-transparent bg-clip-text bg-gradient-to-r from-magma to-taupe">ARCHIVE</span>
            </h2>
          </ConvergenceWrapper>

          <ConvergenceWrapper staggerIndex={1}>
            <div className="flex gap-4 items-center bg-white/5 p-2 rounded-lg border border-white/10">
              ...
              <input 
                ...
                className="bg-transparent border-none outline-none text-bone placeholder-gray-600 font-sans w-64 text-base"
              />
            </div>
          </ConvergenceWrapper>
        </div>
        ...
        <div className="relative mb-32">
          <div 
            className={`grid grid-cols-3 gap-6 ${
              showScrollbar ? 'max-h-[60vh] overflow-y-auto pr-4 pt-4 pb-24' : ''
            }`}
            style={{
              scrollbarWidth: 'thin',
          ...
          {showScrollbar && (
            <div className="absolute bottom-0 left-0 right-4 h-32 bg-gradient-to-t from-obsidian to-transparent pointer-events-none" />
          )}
        </div>
      </div>
    </section>
